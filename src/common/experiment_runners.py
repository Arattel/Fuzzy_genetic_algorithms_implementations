...
def run_experiments_particle_swarm(n_experiments: int = 5, fitness_fn=schwefel, population_scale=500, N=500, epochs=500):
    experiment_logs = []
    for experiment in tqdm(range(n_experiments)):
        history, best_solution, ncalls = particle_swarm_simulation(seed=experiment, 
                                                                   N_genomes=N, N_dimensions=5, N_epochs=epochs, 
...
def run_experiments_standard_GA(n_experiments: int = 5, fitness_fn=schwefel, population_scale=500, mutation_scale=.5, N=500, epochs=500):
    experiment_logs = []
    for experiment in tqdm(range(n_experiments)):
        sim =  GA(fitness_fn=fitness_fn, population_scale=population_scale, mutation_scale=mutation_scale)
