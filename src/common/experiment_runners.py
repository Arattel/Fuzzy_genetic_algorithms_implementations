...
def run_experiments_efga(n_experiments: int = 5, fitness_fn=schwefel, population_scale=500, mutation_scale=.5, N=500, epochs=500, n_terms_params: int = 3, n_terms_priority: int = 3,):
    experiment_logs = []
    for experiment in tqdm(range(n_experiments)):
        history, best_solution, ncalls = efga_simulation(epochs=epochs, fitness_fn=fitness_fn, population_scale=population_scale, mutation_scale=mutation_scale, N=N, 
                                                         seed=experiment, n_terms_params=n_terms_params, n_terms_priority=n_terms_priority)
    ...
    return experiment_logs, {'n_experiments':  n_experiments, 'fitness_fn': fitness_fn.__name__, 
                             ...
                             'n_terms_params': n_terms_params, 'n_terms_priority': n_terms_priority}
